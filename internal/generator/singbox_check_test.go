...
func allProtocolProfile() *model.Profile {
	...
	withQUICTLS := func(e model.Endpoint) model.Endpoint {
		e.TLS = &model.TLS{Enabled: true, Type: "tls", SNI: "wayhop.local", Insecure: true, ALPN: []string{"h3"}}
		return e
	}
	...
	// (allowInsecure=1) on the /wayhop path.
	vmess := generator_singBoxEndpoint("p-vmess", model.ProtoVMess, map[string]any{"uuid": uuid, "alter_id": 0, "security": "auto"})
	vmess.Transport = &model.Transport{Type: "ws", Path: "/wayhop", Host: "wayhop.local"}
	vmess.TLS = &model.TLS{Enabled: true, Type: "tls", SNI: "wayhop.local", Insecure: true, Fragment: true, RecordFragment: true}
	...
	trojan.TLS = &model.TLS{Enabled: true, Type: "tls", SNI: "wayhop.local", Insecure: true}
	...
	anytls.TLS = &model.TLS{Enabled: true, Type: "tls", SNI: "wayhop.local", Insecure: true}
