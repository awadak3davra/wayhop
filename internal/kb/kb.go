...
var entries = []Entry{
	// --- sing-box ---
	{
		...
		Fix:         "Stop any other sing-box/wayhop instance, delete the stale TUN (e.g. `ip link del tun0`), or reboot. Make sure only one router owns the TUN.",
	...
	},
	{
		...
		Fix:         "Validate with `sing-box check -c config.json` (wayhop's Apply does this automatically), then fix the named section or free the busy port.",
	...
	// --- amneziawg ---
	{
		...
		Fix:         "Copy the exact Jc/Jmin/Jmax/S1/S2/H1–H4 from the server. (A plain WireGuard server also works if only Jc/Jmin/Jmax are set and the rest are 0.) Re-import the .conf so wayhop captures every param.",
	...
	// --- provisioned inbound: port conflicts (any inbound on 8443/8444/8388/8445/8446) ---
	{
		...
		Fix:         "Run `ss -tlunp | grep -E '8443|8444|8388|8445|8446'` to find the owner, stop it, then restart sing-box (or choose a different port in the WayHop Init-Server settings).",
	...
	// --- provisioned inbound: TLS — client rejects self-signed cert ---
	{
		...
		Fix:         "Enable 'allow_insecure' / 'insecure=1' on the client (wayhop's Init-Server share links include this flag). Alternatively, replace the self-signed cert with a Let's Encrypt cert matching the SNI domain.",
		Sources:     []string{"https://sing-box.sagernet.org/configuration/shared/tls/", "https://github.com/SagerNet/sing-box/issues/1844"},
	},
	...
	// --- general (any engine) ---
	{
		...
		Fix:         "Use an IP instead of a hostname, point wayhop's DNS at a working DoH/DoT resolver, or check that DNS isn't being intercepted upstream.",
	...
	},
	{
		...
		Fix:         "Sync the clock via NTP. wayhop warns on large clock skew at startup.",
	...
	},
	{
		...
		Fix:         "Run `sing-box check -c <config>` to see the exact bad key/line. An 'unknown field' means the JSON uses a key the installed core (target 1.12.x) doesn't support — re-Apply from WayHop so the JSON matches the core, or update the core.",
	...
	},
	{
		...
		Fix:         "Re-copy the key / short-id from its source (the server config or the share link) and re-import — WayHop normalizes key encodings on import, so importing the link or .conf is more reliable than hand-editing the profile JSON. A short-id must be hex, e.g. `0123abcd`.",
	...
	},
	{
		...
		Fix:         "Install resolvconf/openresolv (`opkg install resolvconf` on Entware), OR remove the 'DNS =' line from the AmneziaWG profile and let WayHop handle DNS, OR set 'Table = off' and point DNS manually via PostUp.",
	...
	// --- WayHop's own local ports + host resource limits ---
	{
		...
		Title:       "WayHop's local proxy / Clash-API port is already in use",
		Explanation: "sing-box couldn't bind WayHop's own local mixed-proxy (default :7890) or Clash-API (default :9090) port — distinct from the provisioned server inbounds (8443/8444/…). Almost always a previous sing-box/WayHop instance that didn't exit cleanly still holds the socket, or another proxy is listening on the same port.",
		Fix:         "Find the holder with `ss -tlnp | grep -E ':(7890|9090)'`. If it's a stale `sing-box`, `killall sing-box` and let WayHop's watchdog restart it cleanly; or change WayHop's mixed/Clash port in Settings if another app needs that port.",
		Sources:     []string{"https://github.com/SagerNet/sing-box/issues/3411", "https://sing-box.sagernet.org/configuration/inbound/"},
	},
	{
