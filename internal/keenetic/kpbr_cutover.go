...
// are asymmetric), installs the WayHop kernel plane + dnsmasq ipset config (dnsmasq restart —
...
type CutoverPaths struct {
	...
	DnsmasqFile   string // default 30-wayhop.conf (within DnsmasqDir)
	FailoverCron  string // default /opt/etc/cron.1min/wr-failover
	NetfilterHook string // default /opt/etc/ndm/netfilter.d/40-wayhop.sh
...
func (p *CutoverPaths) defaults() {
	...
	if p.DnsmasqFile == "" {
		p.DnsmasqFile = "30-wayhop.conf"
	}
	if p.FailoverCron == "" {
		p.FailoverCron = "/opt/etc/cron.1min/wr-failover"
	}
	if p.NetfilterHook == "" {
		p.NetfilterHook = "/opt/etc/ndm/netfilter.d/40-wayhop.sh"
	}
	if p.KeenPBRInit == "" {
...
// NDM config (S86/S89 stay); S82wayhop-pbr re-applies the plane at boot, so WR PERSISTS across
...
func EmitCutoverScript(art *KernelArtifacts, paths CutoverPaths) string {
	...
	b.WriteString("# WayHop kernel-PBR cutover — replaces keen-pbr LIST routing; KEEPS S86+S89.\n")
	...
	b.WriteString("# 3. WayHop kernel plane: ipsets + mangle/nat MARK + ip rule/route\n")
	...
	fmt.Fprintf(&b, "chmod +x %s\n", paths.NetfilterHook)
	heredoc(&b, "/opt/etc/init.d/S82wayhop-pbr", "WRBOOT", BootInitScript(paths))
	b.WriteString("chmod +x /opt/etc/init.d/S82wayhop-pbr\n")
...
// BootInitScript renders the routing boot init (e.g. /opt/etc/init.d/S82wayhop-pbr): at boot
...
func BootInitScript(paths CutoverPaths) string {
	...
	b.WriteString("# WayHop kernel-PBR boot apply. Auto-generated; ENABLED=yes → runs at boot.\n")
...
// EmitRollbackScript renders the device rollback.sh: tear down the WayHop plane and restore
...
func EmitRollbackScript(art *KernelArtifacts, paths CutoverPaths) string {
	...
	var b strings.Builder
	b.WriteString("#!/opt/bin/sh\n")
	b.WriteString("# WayHop kernel-PBR rollback — tear down WR plane, restore keen-pbr.\n")
	fmt.Fprintf(&b, "rm -f %s %s %s /opt/etc/init.d/S82wayhop-pbr\n", paths.FailoverCron, paths.NetfilterHook, dns)
