...
func TestRenderDnsmasqSets_Nftset(t *testing.T) {
	...
	got := RenderDnsmasqSets(sets, DnsmasqOptions{Table: "wayhop_pbr"})
	want := dnsmasqHeader + "\n" +
		"nftset=/example.com/sub.example.com/inet#wayhop_pbr#list_block_4,inet#wayhop_pbr#list_block_6\n"
	if got != want {
...
func TestRenderDnsmasqSets_DefaultTable(t *testing.T) {
	// Empty Table → defaults to wayhop_pbr for the nftset form.
	sets := []DomainSet{{SetBase: "z", Domains: []string{"example.net"}}}
	got := RenderDnsmasqSets(sets, DnsmasqOptions{})
	if !strings.Contains(got, "inet#wayhop_pbr#z_4") {
		t.Fatalf("expected default table wayhop_pbr, got: %q", got)
	}
}
...
func TestRenderDnsmasq_FromCompiledPlan(t *testing.T) {
	...
	if !strings.Contains(snippet, "nftset=/example.com/test.example/inet#wayhop_pbr#list_censored_4") {
