	...
	for _, p := range b.Protocols {
		...
		case initserver.ProtoReality:
			...
			sb.WriteString("WR_CLIENT_CONFIG=vless://11111111-2222-3333-4444-555555555555@" + host +
				":443?security=reality&sni=www.microsoft.com&fp=chrome&pbk=DemoPublicKey0000000000000000000000000000000&sid=abcd1234&flow=xtls-rprx-vision&type=tcp#wakeroute-server\n")
...
func (s *Server) handleServerLockdown(w http.ResponseWriter, r *http.Request) {
	...
	if b.Key == "" && !s.config().Demo {
		writeErr(w, http.StatusBadRequest, "lockdown requires the new SSH key (so wakeroute can verify key-auth works first)")
		return
	}
...
func keyFilename(b hardenReq) string {
	return "wakeroute-" + slug(b.Host) + "-ed25519"
}
