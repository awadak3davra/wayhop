...
// TestMain lets the test binary double as a sanity-runnable WakeRoute binary: SelfUpdate
...
func TestMain(m *testing.M) {
	for _, a := range os.Args[1:] {
		if a == "-version" || a == "--version" {
			os.Stdout.WriteString("wakeroute 9.9.9 (self-update test stub)\n")
			os.Exit(0)
		}
	}
	os.Exit(m.Run())
...
func TestSelfUpdate_PresentDigestVerifiesAndInstalls(t *testing.T) {
	...
	dir := t.TempDir()
	exePath := filepath.Join(dir, "wakeroute")
	...
	if _, err := os.Stat(filepath.Join(dir, ".wakeroute.new")); !os.IsNotExist(err) {
		t.Errorf("leftover .wakeroute.new after swap")
	}
}
...
func TestSelfUpdate_EmptyDigestRefused(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".wakeroute.new")); !os.IsNotExist(err) {
		t.Errorf("a staged .wakeroute.new was written despite refusal")
	}
}
